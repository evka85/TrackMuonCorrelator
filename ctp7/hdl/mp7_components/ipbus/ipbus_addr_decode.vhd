...
package body ipbus_addr_decode is
  ...
  begin
		if    std_match(addr, "----------00------00------------") then
			sel := 0; -- block_ram / base 00000000 / mask 000003ff
		elsif std_match(addr, "----------10--------------------") then
			sel := 1; -- cii / base 00200000 / mask 000fffff
		elsif std_match(addr, "----------01------01------------") then
			sel := 2; -- mpc / base 00101000 / mask 000000ff
		elsif std_match(addr, "----------01------10------------") then
			sel := 3; -- omtf_proc / base 00102000 / mask 000000ff
		elsif std_match(addr, "----------00------1-------------") then
			sel := 4; -- mp7 / base 00002000 / mask 00001fff
		else
			sel := 99;
		end if;
		return sel;
